...
def test_delta_partitioning_by_single_column(spark, tmp_path):
...
def test_delta_constant_partition_uses_multiple_writers(spark, tmp_path):
    ...
    row_count = 32_768
    input_partitions = 4
    df = spark.range(0, row_count, 1, input_partitions).withColumn("day", F.lit("2026-09-06"))
...
def test_delta_partitioning_creates_correct_directory_structure(spark, delta_test_data, tmp_path):
    ...
    expected_data = pd.DataFrame({"event": ["A", "B", "A"], "score": [0.98, 0.54, 0.76], "id": [10, 11, 12]}).astype(
        {"event": "string", "score": "float64", "id": "int32"}
    )
...
def test_delta_append_uses_existing_partition_metadata(spark, tmp_path):
